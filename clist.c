...
bool add_init(clist *l, DATA data){
    ...
    if(is_empty(l)){ 
        ...
        l->head->next = l-> head;// para que sea circular 
        l->num = 1;
        return true;
    }
    node *nuevo = create_node(data);//cuando la lista tiene elementos
    nuevo->next = l ->head;//nuevo apunta a l->head
    l->head = nuevo;//recorremos l->head a nuevo
    node *ult = ultimo(l);
    ult->next = l -> head;
    l ->num ++;
    return true;
